...
import { parseNewsCoverUrlOrEmbed } from "@/lib/news-cover";
...
export async function saveNews(formData: FormData) {
  ...
  if (cover instanceof File && cover.size > 0) {
    ...
    const raw = formData.get("cover_image_url");
    const coverVal =
      raw == null ? null : parseNewsCoverUrlOrEmbed(String(raw), "Обложка");
    if (coverVal) {
      await supabase.from("news").update({ cover_image_url: coverVal }).eq("id", newsId);
    }
  }
